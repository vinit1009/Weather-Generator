...
public class WeatherGenerator1 {
    ...
    int month, double[][] drywet, double[][] wetwet) {
        ...
        int[] dayArray = new int[numberOfDaysInMonth[month -2]];
        int rowNum = 4001;
        double rand = 0.0;
        double rand2 = 0.0;
        ...
        for(int i = 0; i<rowNum; i++){
            if(longitute == drywet[i][0] && latitude == drywet[i][1]){
                for(int j = 1; j<dayArray.length; j++){
                    if(dayArray[j-1] == WET){
                        rand = StdRandom.uniform();
                        rand2 = wetwet[i][month];
                    ...
                    else if(dayArray[j-1] == DRY ){
                        rand = StdRandom.uniform();
                        rand2 = drywet[i][month];
                        if(rand <= rand2){
                            dayArray[j] = WET;
                        }
                        else{
